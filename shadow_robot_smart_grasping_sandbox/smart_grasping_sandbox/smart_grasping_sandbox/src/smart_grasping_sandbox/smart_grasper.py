...
class SmartGrasper(object):
    ...
    def __init__(self):
        ...
        print("#####################################################################")
        self.arm_commander = MoveGroupCommander("arm", wait_for_servers=30.0)
        self.hand_commander = MoveGroupCommander("hand", wait_for_servers=30.0)
        ...
        if self.__hand_traj_client.wait_for_server(timeout=rospy.Duration(30.0)) is False:
        ...
        if self.__arm_traj_client.wait_for_server(timeout=rospy.Duration(30.0)) is False:
    ...
    def move_tip_absolute(self, target):
        ...
        if not self.arm_commander.execute(plan[1]):
            return False
        return True
    ...
    def open_hand(self):
        ...
        if not self.hand_commander.execute(plan[1], wait=True):
            return False
    ...
    def __pre_grasp(self, arm_target):
        self.hand_commander.set_named_target("open")
        plan = self.hand_commander.plan()
        self.hand_commander.execute(plan[1], wait=True)

        for _ in range(10):
            self.arm_commander.set_start_state_to_current_state()
            self.arm_commander.set_pose_targets([arm_target])
            plan = self.arm_commander.plan()
            if self.arm_commander.execute(plan[1]):
                return True
